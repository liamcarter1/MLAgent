...
class TestImageDryRun:
    """Torch is only required for this class; a missing torch skips just these tests,
    not the tabular ones above (which never import it)."""

    @pytest.fixture(autouse=True, scope="class")
    def _require_torch(self):
        pytest.importorskip("torch")

    def test_the_image_dry_run_writes_the_whole_contract(self, clean_image_project):
    ...
    def test_the_image_dry_run_prints_a_human_line(self, clean_image_project):
        root = install(clean_image_project, IMAGE_TEMPLATE, TINY_CNN)
        proc = run(root, "--dry-run")
        assert proc.returncode == 0, proc.stdout + proc.stderr
        lines = [ln for ln in proc.stdout.strip().splitlines() if ln.strip()]
    ...
    def test_the_image_dry_run_leaves_the_model_untrained_on_disk(self, clean_image_project):
        root = install(clean_image_project, IMAGE_TEMPLATE, TINY_CNN)
        assert run(root, "--dry-run").returncode == 0
        assert not (root / "checkpoints").exists() or not any(
            (root / "checkpoints").glob("*.pt"))

    def test_a_broken_image_dry_run_exits_non_zero_with_the_error_on_stderr(
        self, clean_image_project
    ):
        root = install(clean_image_project, IMAGE_TEMPLATE, TINY_CNN)
    ...
    def test_a_plain_image_run_still_ignores_stray_kernel_flags(self):
    ...
    def test_the_image_dry_run_section_is_in_the_walkthrough(self):
        from mlagent.codewalk import split_sections
